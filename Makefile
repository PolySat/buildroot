...
.PHONY: all world dirs clean distclean source outputmakefile \
	...
	$(HOST_DIR) $(BINARIES_DIR) $(STAMP_DIR) rootfs-cleanup installer-images
...
ifneq ($(BR2_HAVE_DOCUMENTATION),y)
	...
	rm -rf $(TARGET_DIR)/usr/local/man $(TARGET_DIR)/usr/local/share/man
	rm -rf $(TARGET_DIR)/usr/info $(TARGET_DIR)/usr/share/info
	rm -rf $(TARGET_DIR)/usr/local/info $(TARGET_DIR)/usr/local/share/info
	rm -rf $(TARGET_DIR)/usr/doc $(TARGET_DIR)/usr/share/doc
	rm -rf $(TARGET_DIR)/usr/local/doc $(TARGET_DIR)/usr/local/share/doc
	rm -rf $(TARGET_DIR)/usr/share/gtk-doc
	-rmdir $(TARGET_DIR)/usr/share 2>/dev/null
endif
ifeq ($(BR2_PACKAGE_PYTHON_PY_ONLY),y)
	find $(TARGET_DIR)/usr/lib/ -name '*.pyc' -print0 | xargs -0 rm -f
	find $(TARGET_DIR)/usr/local/lib/ -name '*.pyc' -print0 | xargs -0 rm -f
endif
ifeq ($(BR2_PACKAGE_PYTHON_PYC_ONLY),y)
	find $(TARGET_DIR)/usr/lib/ -name '*.py' -print0 | xargs -0 rm -f
	find $(TARGET_DIR)/usr/local/lib/ -name '*.py' -print0 | xargs -0 rm -f
endif
