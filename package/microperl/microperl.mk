...
MICROPERL_MODS_DIR = /usr/local/lib/perl5/$(MICROPERL_VERSION)
...
define HOST_MICROPERL_CONFIGURE_CMDS
	...
	./Configure -Dcc="$(HOSTCC)" -Dprefix="$(HOST_DIR)/usr" \
		-Dloclibpth='/lib /lib64 /usr/lib /usr/lib64 /usr/local/lib /usr/local/lib64' -des
endef
...
define MICROPERL_INSTALL_TARGET_CMDS
	$(INSTALL) -m 0755 -D $(@D)/microperl $(TARGET_DIR)/usr/local/bin/microperl
	ln -sf microperl $(TARGET_DIR)/usr/local/bin/perl
...
define MICROPERL_UNINSTALL_TARGET_CMDS
	rm -f $(TARGET_DIR)/usr/local/bin/perl
	rm -f $(TARGET_DIR)/usr/local/bin/microperl
