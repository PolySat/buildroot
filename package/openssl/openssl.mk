...
define OPENSSL_INSTALL_FIXUPS
	rm -f $(TARGET_DIR)/usr/local/bin/c_rehash
	# libraries gets installed read only, so strip fails
	chmod +w $(TARGET_DIR)/usr/local/lib/engines/lib*.so
	for i in $(addprefix $(TARGET_DIR)/usr/local/lib/,libcrypto.so.* libssl.so.*); \
	do chmod +w $$i; done
	rm -f $(addprefix $(TARGET_DIR)/usr/local/lib/,libcrypto.a libssl.a)
endef
