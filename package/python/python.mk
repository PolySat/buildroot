...
PYTHON_CONFIGURE_EXEC_PREFIX=/usr/local
...
define PYTHON_FIXUP_LIBDIR
	$(SED) 's|^LIBDIR=.*|LIBDIR= $(STAGING_DIR)/usr/lib|' \
	   $(STAGING_DIR)/usr/local/lib/python$(PYTHON_VERSION_MAJOR)/config/Makefile
endef
...
define PYTHON_REMOVE_DEVFILES
	rm -f $(TARGET_DIR)/usr/local/bin/python$(PYTHON_VERSION_MAJOR)-config
	rm -f $(TARGET_DIR)/usr/local/bin/python-config
endef
...
define PYTHON_REMOVE_USELESS_FILES
	for i in `find $(TARGET_DIR)/usr/local/lib/python$(PYTHON_VERSION_MAJOR)/config/ \
