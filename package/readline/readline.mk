...
define READLINE_INSTALL_TARGET_CMDS
	...
	chmod 775 $(TARGET_DIR)/usr/local/lib/libreadline.so.$(READLINE_VERSION) \
		$(TARGET_DIR)/usr/local/lib/libhistory.so.$(READLINE_VERSION)
	$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) \
		$(TARGET_DIR)/usr/local/lib/libreadline.so.$(READLINE_VERSION) \
		$(TARGET_DIR)/usr/local/lib/libhistory.so.$(READLINE_VERSION)
endef
