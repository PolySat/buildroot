...
ifeq ($(BR2_GCC_SHARED_LIBGCC),y)
	...
	-$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) $(TARGET_DIR)/usr/local/lib/libstdc++.so*
endif
endif
ifeq ($(BR2_INSTALL_FORTRAN),y)
ifeq ($(BR2_GCC_SHARED_LIBGCC),y)
	mkdir -p $(TARGET_DIR)/usr/local/lib
	-cp -dpf $(HOST_DIR)/usr/$(REAL_GNU_TARGET_NAME)/lib*/libgfortran.so* \
		$(STAGING_DIR)/usr/lib/
	-cp -dpf $(HOST_DIR)/usr/$(REAL_GNU_TARGET_NAME)/lib*/libgfortran.so* \
		$(TARGET_DIR)/usr/local/lib/
	-$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) $(TARGET_DIR)/usr/local/lib/libgfortran.so*
endif
endif
ifeq ($(BR2_GCC_ENABLE_OPENMP),y)
